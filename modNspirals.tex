...
Most interesting to the author is that this process shows a link between an easily repeatable process and the \textit{Greatest Square Divisor} . From above we have $\xi(N, k) = \frac{k^2 N}{GSD(N)}$ and then simply by writing out $Ond^1_N$ you can easily find the $GSD$ for $N$. However, this gets tedious for $N$ getting bigger.
